...
library ChessValidator {
  ...
  function Flags(Flag i) pure internal returns (uint) {
      return uint(uint8(c_Flags[uint(i)]));
  }

  function Pieces(Piece i) pure internal returns (int8) {
      return -6 + int8(uint8(i));
  }

  function Directions(Direction i) pure internal returns (int8) {
      return -64 + int8(uint8(c_Directions[uint(i)]));
  }

  function Players(Player p) pure internal returns (int8) {
      if (p == Player.WHITE) {
          return 1;
      }
      return -1;
   ...
   */
  function getFlag(State storage self, Flag flag) view internal returns (int8) {
      return self.fields[Flags(flag)];
  }

  function setupState(State storage self, int8 nextPlayerColor) internal {
      // Initialize state
      for (uint i = 0; i < 128; i++) {
          // Read defaultState bytes string, which is offset by 8 to be > 0
          self.fields[i] = int8(uint8(defaultState[i])) - 8;
      }
  ...
  function move(State storage self, uint256 fromIndex, uint256 toIndex, bool isWhite) public {
    ...
    if (abs(fromFigure) != uint(uint8(Pieces(Piece.WHITE_KNIGHT)))) {
        // In case of king, it will check that he is not in check on any of the fields he moves over
        bool checkForCheckVar = abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_KING)));
        checkWayFree(self, fromIndex, toIndex, currentPlayerColor, checkForCheckVar);
    ...
    checkLegality(self, fromIndex, toIndex, fromFigure, currentPlayerColor);
    // Update move count
  ...
  function sanityCheck(uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure, int8 currentPlayerColor) pure internal {
  ...
  */
  function validateMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure, int8 movingPlayerColor) internal returns (bool) {
    int direction = int(getDirection(fromIndex, toIndex));
    bool isDiagonal = !(abs(direction) == 16 || abs(direction) == 1);

    // Kings
    if (abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_KING)))) {
        // Normal move
    ...
    if (abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_BISHOP))) ||
        abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_QUEEN))) ||
        abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_ROOK)))) {

        // Bishop can only walk diagonally, Rook only non-diagonally
        if (!isDiagonal && abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_BISHOP))) ||
            isDiagonal && abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_ROOK)))) {
            return false;
        }
    ...
    // Pawns
    if (abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_PAWN)))) {
    ...
    if (abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_KNIGHT)))) {
        for (uint i; i < 8; i++) {
            if (int(fromIndex) + int(int8(uint8(knightMoves[i]))) - 64 == int(toIndex)) {
                return true;
            }
        }
        return false;
  ...
  */
  ...
  function checkForCheck(State storage self, uint256 kingIndex, int8 currentPlayerColor) internal returns (bool) {
    ...
    for (uint dir = 0; dir < 8; dir ++) {
        ...
        int8 firstFigureIndex = getFirstFigure(self, Directions(Direction(dir)), int8(uint8(kingIndex)));

        // if we found a figure in the danger direction
        if (firstFigureIndex != -1) {
            int8 firstFigure = self.fields[uint(uint8(firstFigureIndex))];
            ...
            if (firstFigure * currentPlayerColor < 0) {
                ...
                if (validateMove(self, uint256(uint8(firstFigureIndex)), uint256(kingIndex), firstFigure, kingFigure, currentPlayerColor)) {
                    // it can
    ...
    for (uint8 movev = 0; movev < 8; movev ++){
        // currentMoveIndex: where knight could start with move that checks king
        int8 currentMoveIndex = int8(uint8(kingIndex)) + int8(uint8(knightMoves[movev])) - 64;

        // if inside the board
        if (uint(uint8(currentMoveIndex)) & 0x88 == 0){
  ...
  function makeMove(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 toFigure) internal {
    ...
    if (fromFigure == Pieces(Piece.BLACK_KING)) {
        ...
        setFlag(self, Flag.BLACK_KING_POS, int8(uint8(toIndex)));
        // Castling
    ...
    if (fromFigure == Pieces(Piece.WHITE_KING)) {
        // Update position flag
        setFlag(self, Flag.WHITE_KING_POS, int8(uint8(toIndex)));
        // Castling
    ...
    if (abs(fromFigure) == uint(uint8(Pieces(Piece.WHITE_PAWN)))) {
        ...
        // in case of double Step: set EN_PASSANT-Flag
            ...
            if (fromFigure == Pieces(Piece.BLACK_PAWN)) {
                setFlag(self, Flag.BLACK_EN_PASSANT, int8(uint8(toIndex)) + Directions(Direction.UP));
            } else {
                setFlag(self, Flag.WHITE_EN_PASSANT, int8(uint8(toIndex)) + Directions(Direction.DOWN));
            }
        }
    }
  ...
  function getDirection(uint256 fromIndex, uint256 toIndex) pure internal returns (int8) {
    ...
    if (isAboveLeft) {
      ...
      function checkLegality(State storage self, uint256 fromIndex, uint256 toIndex, int8 fromFigure, int8 movingPlayerColor) internal returns (bool){
        ...
        if (checkForCheck(self, uint(uint8(kingIndex)), movingPlayerColor)) {
            revert();
        }
        ...
        int8 kingDangerDirection = getDirection(uint256(uint8(kingIndex)), fromIndex);
        // get the first Figure in this direction. Threat of Knight does not change through move of fromFigure.
        ...
        if (firstFigureIndex != -1) {
            ...
            if (firstFigure * movingPlayerColor < 0) {
                ...
                if (validateMove(self, uint256(uint8(firstFigureIndex)), uint256(uint8(kingIndex)), firstFigure, kingFigure, movingPlayerColor)) {
                    // it can
                    revert();
                }
            }
    ...
    function getFirstFigure(State storage self, int8 direction, int8 start) view internal returns (int8){
  ...
  function is_diagonal(int8 direction) pure internal returns (bool){
    return !(abs(direction) == 16 || abs(direction) == 1);
  }

  function getOwnKing(State storage self, int8 movingPlayerColor) view internal returns (int8){
  ...
  function boolToInt(bool value) pure internal returns (int) {
      if (value) {
          return 1;
      } else {
          return 0;
